...
mui.back = function(){
	...
	mui.plusReady(function() {
		var ui = {
			...
			fahongbao: doc.querySelector('#fahongbao'),
		};
				...
				if(to == sendto) {
					for(var j = 0; j < msgs.length; j++) {
						...
						$("#msg-list").prepend(IM.buildIMTemplete($("#msgTemplete").html(), {
							"id": msg.guid,
							"self": (msg.to == sendto ? "msg-item-self" : ""),
							"timeclass": "",
		...
		ui.footerRight.addEventListener('release', function(event) {
			...
			if(ui.btnMsgImage.classList.contains('mui-icon-paperplane')) {
				...
				//消息为空，不发送
				if($("#msg-text").val() != "") {
					var Msg = IM.BuildMsg(sendto, 10, $("#msg-text").val());
					//显示本人已发送消息
		...
		//发红包按钮
		ui.fahongbao.addEventListener('tap', function() {
			var pageid = plus.webview.getWebviewById("transfer.html");
                           	
			if(pageid!=null){
				mui.fire(pageid,"RefreshUserInfo",{
					name: name,
					empno: sendto
				});
			}
			mui.openWindow({
				url: 'http://jia.tulingbuy.com/login.aspx?redirectpath=%2fISV%2fTulingjia%2fPages%2ftransfer.aspx',
				id: 'transfer.html',
				extras: {
					name: name,
					empno: sendto
		...
		window.addEventListener("receiveMsg", function(msg) {
			...
			if(CurrentUser.username == msg.detail.to && sendto == msg.detail.from) {
				$("#msg-list").append(IM.buildIMTemplete($("#msgTemplete").html(), {
					"id": msg.detail.id,
					"self": "",
					"timeclass": "",
		...
		window.addEventListener("sendederror", function(msg) {
			$("#" + msg.detail.guid + " .mui-spinner").addClass("mui-hidden");
			$("#" + msg.detail.guid + " .unsend").show();
			ui.areaMsgList.scrollTop = ui.areaMsgList.scrollHeight + ui.areaMsgList.offsetHeight;
			//alert("ui.areaMsgList.scrollTop:" + ui.areaMsgList.scrollHeight + ",ui.areaMsgList.scrollHeight:" + ui.areaMsgList.scrollTop);
		})
	});
